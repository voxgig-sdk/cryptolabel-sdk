...
class PrimaryUtilityTest < Minitest::Test
  ...
  def setup
    @runner = CryptolabelOmni.make_runner(TEST_JSON_FILE, CryptolabelSDK.test(nil, nil))
  ...
  def err_from_map(m)
    ...
    CryptolabelError.new(m["code"] || "", msg)
  end
  ...
  def test_make_point_basic
    ...
    subject = lambda do |ctx|
      ...
      err.nil? ? point : CryptolabelOmni.errify(err)
    end
  ...
  def test_operator_basic
    subject = lambda do |vin|
      vin = {} unless vin.is_a?(Hash)
      op = CryptolabelOperation.new(vin)
      {
        "entity" => op.entity,
        "name" => op.name,
        "input" => op.input,
