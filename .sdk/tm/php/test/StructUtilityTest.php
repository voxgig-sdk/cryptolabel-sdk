...
// Cryptolabel SDK struct utility test
//
...
class StructUtilityTest extends TestCase
{
    ...
    {
        if (self::$run === null) {
            $runner = CryptolabelOmni::makeStructRunner(
        ...
        $this->runsection(
            ...
            function ($input) {
                ...
                if (is_object($store) && $haspath && Struct::undef() !== $val) {
                    ...
                    if (json_encode(CryptolabelOmni::structfix($now, Struct::undef()))
                        !== json_encode(CryptolabelOmni::structfix($val, Struct::undef()))) {
