...
class FtCtrl extends CryptolabelControl
{
...
class FtClient
{
    ...
    private CryptolabelUtility $utility;

    public function __construct(CryptolabelUtility $utility, array $options)
    {
    ...
    public function get_utility(): CryptolabelUtility
    {
        return $this->utility;
    }
}
...
class FtRecorder
{
    ...
    {
        return function (CryptolabelContext $_ctx, string $url, array $fetchdef): array {
...
class FtHarness
{
    ...
    public CryptolabelContext $rootctx;
    ...
    public function __construct(
        ...
        $utility->fetcher = $server ?? function (CryptolabelContext $_ctx, string $_url, array $fetchdef): array {
        ...
        foreach ($features as $fspec) {
            ...
            $f = CryptolabelConfig::make_feature($name);
    ...
    private static function buildUrl(CryptolabelSpec $spec): string
    {
        $keys = [];
    ...
    public function op(array $o = []): array
    {
        ...
        if (($ctx->out['point'] ?? null) instanceof CryptolabelError) {
        ...
        if (is_array($fetched)) {
            $ctx->response = new CryptolabelResponse($fetched);
        }
    ...
    private function fail(CryptolabelContext $ctx, mixed $err): array
    {
...
class FeatureTest extends TestCase
{
    ...
    public function test_retry_retries_a_transport_error_until_exhausted(): void
    {
        ...
        $clock = new FtClock();
        $n = 0;
        $server = function (CryptolabelContext $ctx, string $_url, array $_fd) use (&$n): array {
            $n++;
