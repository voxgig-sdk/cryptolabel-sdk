...
{
    private static function utility(): CryptolabelUtility
    {
        return new CryptolabelUtility();
    }
    ...
    private static function ctx(array $over = []): CryptolabelContext
    {
        $utility = $over['utility'] ?? self::utility();
        $client = $over['client'] ?? new PlClient(['base' => 'http://h']);
        $ctx = new CryptolabelContext([
            'client' => $client,
            'utility' => $utility,
        ], null);
        $ctx->op = new CryptolabelOperation(['name' => 'load', 'entity' => 'x']);
        foreach ($over as $k => $v) {
    ...
    {
        return ($err instanceof CryptolabelError) ? $err->sdk_code : '';
    }
    ...
    public function test_make_point_rejects_a_disallowed_operation(): void
    {
        ...
        $ctx->op = new CryptolabelOperation(['name' => 'nope', 'entity' => 'x']);
        [$point, $err] = CryptolabelMakePoint::call($ctx);
    ...
    public function test_make_point_rejects_an_operation_with_no_endpoints(): void
    {
        ...
        $ctx->op = new CryptolabelOperation(['name' => 'load', 'entity' => 'x', 'points' => []]);
        [$point, $err] = CryptolabelMakePoint::call($ctx);
        $this->assertNull($point);
    ...
    public function test_make_point_returns_the_single_point(): void
    {
        ...
        $ctx = self::ctx(['options' => ['allow' => ['op' => 'load,list,create,update,remove']]]);
        $ctx->op = new CryptolabelOperation(['name' => 'load', 'entity' => 'x', 'points' => [$point]]);
        [$got, $err] = CryptolabelMakePoint::call($ctx);
        $this->assertNull($err);
    ...
    {
        $preset = new CryptolabelSpec(['method' => 'GET']);
        $ctx = self::ctx();
        $ctx->out['spec'] = $preset;
        [$got, $err] = CryptolabelMakeSpec::call($ctx);
        $this->assertNull($err);
        $this->assertSame($preset, $got);
    }
    ...
    public function test_make_response_guards_missing_spec_response_result(): void
    {
        $ctx = self::ctx([
            'spec' => null,
            'response' => new CryptolabelResponse([]),
            'result' => new CryptolabelResult([]),
        ]);
        [, $err] = CryptolabelMakeResponse::call($ctx);
        $this->assertSame('response_no_spec', self::code($err));

        $ctx = self::ctx([
            'spec' => new CryptolabelSpec([]),
            'response' => null,
            'result' => new CryptolabelResult([]),
        ]);
        [, $err] = CryptolabelMakeResponse::call($ctx);
    ...
    {
        $ctx = self::ctx([
            'spec' => new CryptolabelSpec(['step' => 's']),
            'response' => new CryptolabelResponse(self::resp(404, null, ['x-a' => '1'])),
            'result' => new CryptolabelResult([]),
        ]);
        [, $err] = CryptolabelMakeResponse::call($ctx);
        $this->assertNull($err);
    ...
    public function test_make_response_2xx_parses_the_body_and_marks_ok(): void
    {
        $ctx = self::ctx([
            'spec' => new CryptolabelSpec(['step' => 's']),
            'response' => new CryptolabelResponse(self::resp(200, ['v' => 1])),
            'result' => new CryptolabelResult([]),
        ]);
        [, $err] = CryptolabelMakeResponse::call($ctx);
        $this->assertNull($err);
    ...
    public function test_make_response_records_to_ctrl_explain_when_explain_is_on(): void
    {
        $ctx = self::ctx([
            'spec' => new CryptolabelSpec(['step' => 's']),
            'response' => new CryptolabelResponse(self::resp(200, ['v' => 2])),
            'result' => new CryptolabelResult([]),
        ]);
        $ctx->ctrl->explain = ['on' => true];
        CryptolabelMakeResponse::call($ctx);
    ...
    {
        $preset = new CryptolabelResponse(self::resp(299));
        $ctx = self::ctx([
            'spec' => new CryptolabelSpec([]),
            'response' => new CryptolabelResponse([]),
            'result' => new CryptolabelResult([]),
        ...
        [$got, $err] = CryptolabelMakeResponse::call($ctx);
        $this->assertNull($err);
        $this->assertSame($preset, $got);
    }
    ...
    {
        $ctx = self::ctx(['spec' => null, 'result' => new CryptolabelResult([])]);
        [, $err] = CryptolabelMakeResult::call($ctx);
        $this->assertSame('result_no_spec', self::code($err));

        $ctx = self::ctx(['spec' => new CryptolabelSpec([]), 'result' => null]);
        [, $err] = CryptolabelMakeResult::call($ctx);
    ...
    public function test_make_result_list_op_wraps_resdata_into_entity_instances(): void
    {
        ...
        $ctx = self::ctx([
            'entity' => $entity,
            'spec' => new CryptolabelSpec(['step' => 's']),
            'result' => new CryptolabelResult(['ok' => true, 'resdata' => [['a' => 1], ['a' => 2]]]),
        ]);
        $ctx->op = new CryptolabelOperation(['name' => 'list', 'entity' => 'x']);
        [$result, $err] = CryptolabelMakeResult::call($ctx);
        $this->assertNull($err);
    ...
    public function test_make_result_empty_list_yields_empty_resdata(): void
    {
        ...
        $ctx = self::ctx([
            'entity' => $entity,
            'spec' => new CryptolabelSpec(['step' => 's']),
            'result' => new CryptolabelResult(['ok' => true, 'resdata' => []]),
        ]);
        $ctx->op = new CryptolabelOperation(['name' => 'list', 'entity' => 'x']);
        [$result, $err] = CryptolabelMakeResult::call($ctx);
        $this->assertNull($err);
    ...
    {
        $preset = new CryptolabelResult(['ok' => true]);
        $ctx = self::ctx([
            'spec' => new CryptolabelSpec([]),
            'result' => new CryptolabelResult([]),
        ]);
        $ctx->out['result'] = $preset;
        [$got, $err] = CryptolabelMakeResult::call($ctx);
        $this->assertNull($err);
        $this->assertSame($preset, $got);
    }
    ...
    public function test_make_request_guards_a_missing_spec(): void
    {
        ...
        [, $err] = CryptolabelMakeRequest::call($ctx);
    ...
    {
        $utility = self::utility();
        $boom = new CryptolabelError('boom', 'boom');
        $utility->fetcher = function (CryptolabelContext $_c, string $_u, array $_f) use ($boom): array {
            return [null, $boom];
        };
        $ctx = self::ctx([
            'utility' => $utility,
            'spec' => new CryptolabelSpec(['step' => 's', 'method' => 'GET', 'base' => 'http://h', 'parts' => ['a']]),
        ]);
        [$response, $err] = CryptolabelMakeRequest::call($ctx);
        $this->assertNull($err);
    ...
    {
        $utility = self::utility();
        $utility->fetcher = function (CryptolabelContext $_c, string $_u, array $_f): array {
            return [null, null];
        };
        $ctx = self::ctx([
            'utility' => $utility,
    ...
    public function test_make_request_a_normal_transport_response_is_wrapped(): void
    {
        ...
        [$response, $err] = CryptolabelMakeRequest::call($ctx);
        $this->assertNull($err);
        $this->assertInstanceOf(CryptolabelResponse::class, $response);
    ...
    public function test_make_request_records_the_fetchdef_to_ctrl_explain(): void
    {
        ...
        $ctx->ctrl->explain = ['on' => true];
        CryptolabelMakeRequest::call($ctx);
    ...
    {
        $utility = self::utility();
        $utility->make_fetch_def = function (CryptolabelContext $c): array {
            return [null, $c->make_error('fetchdef_boom', 'boom')];
        };
        $ctx = self::ctx([
            'utility' => $utility,
            'spec' => new CryptolabelSpec(['step' => 's', 'method' => 'GET']),
        ]);
        [$response, $err] = CryptolabelMakeRequest::call($ctx);
        $this->assertNull($err);
        $this->assertNotNull($response->err);
    ...
    {
        $preset = new CryptolabelResponse(self::resp(201));
        $ctx = self::ctx(['spec' => new CryptolabelSpec([])]);
        $ctx->out['request'] = $preset;
        [$got, $err] = CryptolabelMakeRequest::call($ctx);
        $this->assertNull($err);
        $this->assertSame($preset, $got);
    }
    ...
    {
        $ctx = self::ctx(['spec' => null]);
        [, $err] = CryptolabelMakeFetchDef::call($ctx);
    ...
    public function test_make_fetch_def_serialises_body_and_inits_missing_result(): void
    {
        ...
        [$fetchdef, $err] = CryptolabelMakeFetchDef::call($ctx);
        $this->assertNull($err);
    ...
    {
        $ctx = self::ctx(['result' => new CryptolabelResult(['ok' => true, 'resdata' => 42])]);
        $this->assertSame(42, CryptolabelDone::call($ctx));
    }

    public function test_done_raises_the_error_when_not_ok(): void
    {
        $ctx = self::ctx(['result' => new CryptolabelResult(['ok' => false])]);
        $this->expectException(CryptolabelError::class);
        CryptolabelDone::call($ctx);
    }

    public function test_make_error_returns_resdata_when_ctrl_throw_is_false(): void
    {
        $ctx = self::ctx(['result' => new CryptolabelResult(['ok' => false, 'resdata' => 'fallback'])]);
        $ctx->ctrl->throw_err = false;
        $this->assertSame('fallback', CryptolabelMakeError::call($ctx, null));
    }
    ...
    public function test_make_error_records_to_ctrl_explain(): void
    {
        ...
        $ctx->ctrl->throw_err = false;
        $ctx->ctrl->explain = ['on' => true];
        CryptolabelMakeError::call($ctx, null);
    ...
    public function test_feature_add_appends_in_call_order(): void
    {
        ...
        $a = new CryptolabelBaseFeature();
        $b = new CryptolabelBaseFeature();
        CryptolabelFeatureAdd::call($ctx, $a);
        CryptolabelFeatureAdd::call($ctx, $b);
        $this->assertSame([$a, $b], $client->features);
    }

    private static function named_feature(string $name): CryptolabelBaseFeature
    {
        $f = new CryptolabelBaseFeature();
        $f->name = $name;
        return $f;
    }
    ...
    public function test_feature_add_ordering_before_after_replace(): void
    {
        ...
        CryptolabelFeatureAdd::call($ctx, $miss);
    ...
    private static function auth_ctx(array $options, ?array $headers): CryptolabelContext
    {
        $client = new PlClient($options);
        return self::ctx([
            'client' => $client,
            'spec' => $headers === null ? null : new CryptolabelSpec(['headers' => $headers]),
        ]);
    }
    ...
    public function test_prepare_auth_guards_a_missing_spec(): void
    {
        ...
        [, $err] = CryptolabelPrepareAuth::call($ctx);
    ...
    public function test_prepare_auth_a_raw_apikey_goes_in_as_is(): void
    {
        ...
        CryptolabelPrepareAuth::call($ctx);
    ...
    {
        $ctx = self::ctx([
            'response' => new CryptolabelResponse(['headers' => null]),
            'result' => new CryptolabelResult([]),
        ]);
        CryptolabelResultHeaders::call($ctx);
    ...
    public function test_result_body_skips_parsing_when_the_body_is_absent(): void
    {
        ...
        CryptolabelResultBody::call($ctx);
