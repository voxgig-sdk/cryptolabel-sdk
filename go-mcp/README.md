...
[MCP](https://modelcontextprotocol.io) server exposing the Cryptolabel SDK as
two agent tools — `cryptolabel_list` and `cryptolabel_load` — built on the
...
```sh
# 1. Build a native binary (-> dist/<os>-<arch>/cryptolabel-mcp)
make build
...
claude mcp add --scope user cryptolabel \
  -- /absolute/path/to/cryptolabel-mcp -transport stdio

# 3b. …or run a shared HTTP server instead
./cryptolabel-mcp -transport http -addr :8080
```

Tool-call arguments (what an agent sends):

```jsonc
// cryptolabel_list: first page of records
{ "entity": "address" }
{ "entity": "address", "query": { } }

// cryptolabel_load: one record by id
...
1. **Build** the server from this `go-mcp/` directory:
   ...
   ```sh
   make build          # -> dist/<os>-<arch>/cryptolabel-mcp
   ```
...
3. **Install it into Claude Code** (stdio transport):
   ...
   ```sh
   claude mcp add --scope user cryptolabel \
     -- "$PWD"/dist/*/cryptolabel-mcp -transport stdio
   ```

4. **Restart Claude Code.** The `cryptolabel_list` and `cryptolabel_load` tools now appear
   in new sessions. Ask the agent to *"list address using cryptolabel"*
   and it calls `cryptolabel_list` with `{"entity":"address"}`.
...
### Call the `cryptolabel_list` tool
...
### Call the `cryptolabel_load` tool
...
| Tool | Args | Returns |
|------|------|---------|
| `cryptolabel_list` | `entity` (required), `query` (optional map) | First page of records as JSON |
| `cryptolabel_load` | `entity` (required), `query` = `{id:N}` | Single record as JSON |
...
| Variable | Purpose |
|----------|---------|
| `CRYPTOLABEL_APIKEY` | API key sent with every request. |
| `CRYPTOLABEL_BASE` | Optional override of the API base URL. |
...
```sh
./cryptolabel-mcp -transport http -addr :18080 &
...
curl -sN -X POST http://localhost:18080 \
  ...
  -d '{"jsonrpc":"2.0","id":2,"method":"tools/call","params":{"name":"cryptolabel_load","arguments":{"entity":"address","query":{"id":1}}}}'
```
