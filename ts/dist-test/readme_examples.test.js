...
const TEST_SEED = { "entity": { "address": { "test01": { "id": "test01" } } } };
...
function isRunnable(code) {
    return (/new\s+CryptolabelSDK\b/.test(code) ||
        /\bCryptolabelSDK\.test\b/.test(code) ||
        /\bclient\s*\./.test(code));
}
...
function rewriteForRun(code) {
    ...
    out = out.replace(/new\s+CryptolabelSDK\s*\([^)]*\)/g, () => SEEDED_CTOR);
    out = out.replace(/CryptolabelSDK\.test\s*\([^)]*\)/g, () => SEEDED_CTOR);
