...
describe('AddressDirect', async () => {
  ...
  afterEach(liveDelay('CRYPTOLABEL_TEST_LIVE'))

  test('direct-exists', async () => {
    const sdk = new CryptolabelSDK({
  ...
  test('direct-load-address', async (t: any) => {
    if (liveScenariosActive()) { t.skip('Covered by live operation scenarios'); return }
    const setup = directSetup({ id: 'direct01' })
    if (maybeSkipControl(t, 'direct', 'direct-load-address', setup.live)) return
    if (skipIfMissingIds(t, setup, ["address01","chain01"])) return
...
function directSetup(mockres?: any) {
  ...
  if (live) {
    ...
    let idmap: any = env['CRYPTOLABEL_TEST_ADDRESS_ENTID']
