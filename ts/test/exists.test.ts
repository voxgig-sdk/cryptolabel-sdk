...
describe('exists', async () => {
  ...
  //
  // CryptolabelSDK.test() is synchronous — it returns the client, not a promise
  ...
  test('test-mode', () => {
    const testsdk = CryptolabelSDK.test()
    equal(testsdk instanceof CryptolabelSDK, true,
      'CryptolabelSDK.test() must return a client synchronously')
  })
