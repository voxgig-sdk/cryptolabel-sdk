...
AQL-driven command-line client **and** interactive REPL for the Cryptolabel
...
```sh
# 1. Build a native binary (-> dist/<os>-<arch>/cryptolabel-cli)
make build

# 2. See usage (words, entities, env vars)
./cryptolabel-cli --help
...
1. **Build the binary.** From this `go-cli/` directory:
   ...
   ```sh
   make build          # -> dist/<os>-<arch>/cryptolabel-cli
   ```
...
Run with no arguments to open a REPL (prompt `cryptolabel>`). Each line is
...
- `<entity>` is a bareword, auto-quoted as an AQL atom (e.g. `address`).
...
| Target | Result |
|--------|--------|
| `make build` | Native binary at `dist/<os>-<arch>/cryptolabel-cli`. |
...
(`./cryptolabel-cli <expr>`) and interactively (the REPL), and expressions compose the
