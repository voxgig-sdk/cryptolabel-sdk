// Command Cryptolabel-cli is a CLI + REPL for the Cryptolabel SDK,
...
const prompt = "cryptolabel"

// entitiesHelp is the space-separated entity list shown by /help.
const entitiesHelp = "address"
...
func usage(out io.Writer) {
	...
	fmt.Fprintln(out, "Env:      CRYPTOLABEL_APIKEY (api key), CRYPTOLABEL_BASE (base url override)")
...
func run(args []string, in io.Reader, out, errOut io.Writer) int {
	...
	// Configure from the environment: CRYPTOLABEL_APIKEY carries the API key and
	// CRYPTOLABEL_BASE optionally overrides the API base URL (e.g. production).
	// Both injectable by a secrets vault. Unset -> nil config defaults.
	var opts map[string]any
	if apikey := os.Getenv("CRYPTOLABEL_APIKEY"); apikey != "" {
		opts = map[string]any{"apikey": apikey}
	}
	if base := os.Getenv("CRYPTOLABEL_BASE"); base != "" {
		if opts == nil {
			opts = map[string]any{}
		}
		opts["base"] = base
	}
	client := sdk.NewCryptolabelSDK(opts)
