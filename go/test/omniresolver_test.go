...
import (
	...
	omni "github.com/voxgig-sdk/cryptolabel-sdk/go/test/omni"
...
func omniProvider(client any) *omni.Provider {
	provider := &omni.Provider{
		...
		Errify: func(err any) any {
			if sdkerr, is := err.(*sdk.CryptolabelError); is {
				out := map[string]any{
					"name":    "CryptolabelError",
...
func omniCtx(arg any, client *sdk.CryptolabelSDK, utility *sdk.Utility) *sdk.Context {
	...
	if provider, is := ctxmap["client"].(*omni.Provider); is {
		if live, is := defproviderclient(provider).(*sdk.CryptolabelSDK); is {
			client = live
