...
func TestAddressDirect(t *testing.T) {
	t.Run("direct-load-address", func(t *testing.T) {
		setup := addressDirectSetup(map[string]any{"id": "direct01"})
		_mode := "unit"
		if setup.live {
			_mode = "live"
		}
		if _shouldSkip, _reason := isControlSkipped("direct", "direct-load-address", _mode); _shouldSkip {
			if _reason == "" {
		...
		if setup.live {
			for _, _liveKey := range []string{"address01", "chain01"} {
...
func addressDirectSetup(mockres any) *addressDirectSetupResult {
	...
	env := envOverride(map[string]any{
		"CRYPTOLABEL_TEST_ADDRESS_ENTID": map[string]any{},
	...
	if live {
		...
		client := sdk.NewCryptolabelSDK(mergedOpts)

		idmap := map[string]any{}
		if entidRaw, ok := env["CRYPTOLABEL_TEST_ADDRESS_ENTID"]; ok {
