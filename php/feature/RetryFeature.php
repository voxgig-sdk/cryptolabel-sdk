...
class CryptolabelRetryFeature extends CryptolabelBaseFeature
{
    ...
    public function init(CryptolabelContext $ctx, array $options): void
    {
        ...
        $utility->fetcher = function (CryptolabelContext $ctx2, string $url, array $fetchdef) use ($inner): array {
            return $this->_with_retry($ctx2, $url, $fetchdef, $inner);
        };
    }

    private function _with_retry(CryptolabelContext $ctx, string $url, array $fetchdef, callable $inner): array
    {
        $opts = $this->options ?? [];
    ...
    private function _track(CryptolabelContext $_ctx, int $attempt, mixed $res, mixed $err, float $wait): void
    {
        $client = $this->client;
