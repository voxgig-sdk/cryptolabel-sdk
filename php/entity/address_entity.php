...
// Cryptolabel SDK Address entity
...
class AddressEntity
{
    ...
    {
        $opts = $this->_entopts;
        return new AddressEntity($this->_client, $opts);
    }

    /**
     * @param Address|array $args Address data (assoc-array) to store.
     */
    public function data_set($args): void
    {
        if ($args) {
            $this->_data = CryptolabelHelpers::to_map(Struct::clone($args)) ?? [];
            ($this->_utility->feature_hook)($this->_entctx, "SetData");
        }
    }

    /**
     * @return Address|array The current Address data as an assoc-array.
     */
    ...
    /**
     * @param array $args Match filter (any subset of Address fields).
     */
    public function match_set($args): void
    {
        if ($args) {
            $this->_match = CryptolabelHelpers::to_map(Struct::clone($args)) ?? [];
            ($this->_utility->feature_hook)($this->_entctx, "SetMatch");
        }
    }

    /**
     * @return array The current match filter (any subset of Address fields).
     */
     ...
     *
     * @param AddressLoadMatch|array|null $reqmatch Match criteria (id/query
     *   fields) as an assoc-array; a typed AddressLoadMatch names the shape.
     * @param mixed $ctrl Optional per-call control overrides.
     * @return Address|array The loaded Address as an assoc-array at the
     *   SDK boundary; throws CryptolabelError on failure (item-5 convention).
     */
        ...
        return $this->_run_op($ctx, function () use ($ctx) {
            if ($ctx->result) {
                ...
                if ($ctx->result->resdata) {
                    $this->_data = CryptolabelHelpers::to_map(Struct::clone($ctx->result->resdata)) ?? [];
                }
            }
        });
    }
