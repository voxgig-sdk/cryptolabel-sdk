...
CryptoLabel API client, generated from the OpenAPI spec.

Public API for resolving crypto addresses into presentation entities and labels.

Learn more about CryptoLabel API at [cryptolabel.io](https://cryptolabel.io).

This is an unofficial SDK for the CryptoLabel public API, generated by Voxgig with [`@voxgig/sdkgen`](https://github.com/voxgig/sdkgen). It is not affiliated with, endorsed by, or sponsored by the upstream API provider.
...
```bash
cd go-mcp && go build -o cryptolabel-mcp .
```
...
```json
{
  "mcpServers": {
    "cryptolabel": {
      "command": "/abs/path/to/cryptolabel-mcp"
    }
  }
}
```
...
```go
import sdk "github.com/voxgig-sdk/cryptolabel-sdk/go"
...
Generated from the CryptoLabel API OpenAPI spec by [@voxgig/sdkgen](https://github.com/voxgig/sdkgen).
