...
from cryptolabel_sdk.core.helpers import get_ctx_prop, to_map
...
class CryptolabelContext:
    def __init__(self, ctxmap=None, basectx=None):
        ...
        if isinstance(sp, CryptolabelSpec):
            self.spec = sp
        elif basectx is not None:
        ...
        if isinstance(r, CryptolabelResult):
            self.result = r
        elif basectx is not None:
        ...
        if isinstance(rp, CryptolabelResponse):
            self.response = rp
        elif basectx is not None:
    ...
    def make_error(self, code, msg):
        return CryptolabelError(code, msg, self)
