...
from cryptolabel_sdk.cryptolabel_types import (
    Address,
    AddressListMatch,
)
...
class AddressEntity:
    ...
    def make(self):
        ...
        for k, v in self._entopts.items():
            opts[k] = v
        return AddressEntity(self._client, opts)
    ...
    def list(self, reqmatch=None, ctrl=None) -> list[Address]:
        utility = self._utility
        # reqmatch is optional: an omitted match lists all records. Treat None
        # as an empty match so client.Address().list() works with no args.
        if reqmatch is None:
            reqmatch = {}
