...
def _address_basic_setup(extra):
    runner.load_env_local()

    entity_data_file = os.path.join(_TEST_DIR, "../../.sdk/test/entity/address/AddressTestData.json")
    ...
    idmap_resolved = helpers.to_map(
        env.get("CRYPTOLABEL_TEST_ADDRESS_ENTID"))
    ...
    if env.get("CRYPTOLABEL_TEST_LIVE") == "TRUE":
        ...
        client = CryptolabelSDK(helpers.to_map(merged_opts))

    _live = env.get("CRYPTOLABEL_TEST_LIVE") == "TRUE"
    return {
        "client": client,
        "data": entity_data,
        "idmap": idmap_resolved,
        "env": env,
        "explain": env.get("CRYPTOLABEL_TEST_EXPLAIN") == "TRUE",
        "live": _live,
