...
module CryptolabelUtilities
  MakeRequest = ->(ctx) {
    ...
    spec = ctx.spec
    utility = ctx.utility
    response = CryptolabelResponse.new({})
    ...
    if fetch_err
      ...
      response = CryptolabelResponse.new({ "err" => ctx.make_error("request_no_response", "response: undefined") })
    elsif fetched.is_a?(Hash)
      response = CryptolabelResponse.new(fetched)
    else
