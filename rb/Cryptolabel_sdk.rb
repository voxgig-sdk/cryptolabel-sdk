...
require_relative 'Cryptolabel_types'


class CryptolabelSDK
  ...
  def initialize(options = {})
    ...
    feature_opts = CryptolabelHelpers.to_map(VoxgigStruct.getprop(@options, "feature"))
    if feature_opts
      featureorder = VoxgigStruct.getpath(@options, "__derived__.featureorder")
      if featureorder.is_a?(Array)
        featureorder.each do |fname|
          fopts = CryptolabelHelpers.to_map(feature_opts[fname])
          if fopts && fopts["active"] == true
            utility.feature_add.call(@_rootctx, CryptolabelFeatures.make_feature(fname))
          end
        end
      end
    end
  ...
  def get_utility
    CryptolabelUtility.copy(@_utility)
  end
  ...
  def prepare(fetchargs = {})
    ...
    ctrl = CryptolabelHelpers.to_map(VoxgigStruct.getprop(fetchargs, "ctrl")) || {}
    ...
    params = CryptolabelHelpers.to_map(VoxgigStruct.getprop(fetchargs, "params")) || {}
    query = CryptolabelHelpers.to_map(VoxgigStruct.getprop(fetchargs, "query")) || {}
  ...
  def op_denied(op)
    ...
    {
      "ok" => false,
      "err" => CryptolabelError.new(
        "#{op}_allow",
        "CryptolabelSDK: #{op}: operation not allowed by" \
  ...
  def raw_request(fetchargs = {})
    ...
    if fetched.is_a?(Hash)
      status = CryptolabelHelpers.to_int(VoxgigStruct.getprop(fetched, "status"))
  ...
  def graphql(query, variables = nil, ctrl = nil)
    ...
    if errors.is_a?(Array) && !errors.empty?
      ...
      res["ok"] = false
      res["err"] = CryptolabelError.new(
        "graphql_error", "CryptolabelSDK: graphql: #{msg}")
      res["graphql"] = errors
    end
  ...
  def self.test(testopts = nil, sdkopts = nil)
    ...
    sdk = CryptolabelSDK.new(sdkopts)
    sdk.mode = "test"
    sdk
  end
end
