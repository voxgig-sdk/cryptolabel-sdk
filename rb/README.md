...
```ruby
require_relative "Cryptolabel_sdk"
client = CryptolabelSDK.new(options)
```
...
```ruby
client = CryptolabelSDK.test(testopts, sdkopts)
```
...
raise a `CryptolabelError` (a `StandardError` subclass), so wrap
...
```
rb/
├── Cryptolabel_sdk.rb       -- Main SDK module
...
The main module (`Cryptolabel_sdk`) exports the SDK class
