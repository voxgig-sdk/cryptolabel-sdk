...
class AddressEntity
  ...
  def make
    opts = @_entopts.dup
    AddressEntity.new(@_client, opts)
  end

  def data_set(args)
    if args
      @_data = CryptolabelHelpers.to_map(VoxgigStruct.clone(args)) || {}
      @_utility.feature_hook.call(@_entctx, "SetData")
    end
  end

  # @return [Address, Hash] the current Address data
  def data_get
  ...
  def match_set(args)
    if args
      @_match = CryptolabelHelpers.to_map(VoxgigStruct.clone(args)) || {}
      @_utility.feature_hook.call(@_entctx, "SetMatch")
    end
  end

  # @return [Hash] the current match filter (any subset of Address fields)
  def match_get
  ...
  #
  # @param reqmatch [AddressLoadMatch, Hash, nil] match criteria (id/query fields);
  #   optional — an entity with no id-like key loads with no match (nil is treated
  #   as an empty match, so client.Address.load works with no args).
  # @param ctrl [Object, nil] optional per-call control
  # @return [Address, Hash] the loaded Address; raises CryptolabelError on failure
    ...
    _run_op(ctx) do
      if ctx.result
        ...
        if ctx.result.resdata
          @_data = CryptolabelHelpers.to_map(VoxgigStruct.clone(ctx.result.resdata)) || {}
        end
      end
    end
  end
