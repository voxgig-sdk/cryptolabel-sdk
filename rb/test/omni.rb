...
module CryptolabelOmni
  ...
  class ObjView < ::Hash
    ...
    def view_wrap(name)
      ...
      return val if CryptolabelOmni.plainval?(val) || seen.key?(val.object_id)
    ...
    def view_attrname(key)
      ...
      snake = CryptolabelOmni.underscore(name)
    ...
    def keys
      out = view_names.map { |n| CryptolabelOmni.camelize(n) }
    ...
    def []=(key, val)
      ...
      t = @view_target
      name = view_attrname(key) || CryptolabelOmni.underscore(key.to_s)
      setter = "#{name}="
  ...
  def enrich(ctxmap, ctx)
    ...
    ctx.spec = CryptolabelSpec.new(spec_map) if spec_map.is_a?(::Hash)
    ...
    if res_map.is_a?(::Hash)
      ...
      if err_map.is_a?(::Hash) && err_map['message'].is_a?(::String) &&
        ...
        ctx.result.err = CryptolabelError.new('', err_map['message'])
      end
    end
  ...
  def sdkhooks(sdk)
    ...
    errify = ->(err) { CryptolabelOmni.errify(err) }
