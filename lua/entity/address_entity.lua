...
local AddressEntity = {}
AddressEntity.__index = AddressEntity


function AddressEntity.new(client, entopts)
  entopts = entopts or {}
  ...
  local self = setmetatable({}, AddressEntity)
  self._name = "address"
  self._client = client
...
function AddressEntity:get_name()
  return self._name
end


function AddressEntity:make()
  local opts = {}
  for k, v in pairs(self._entopts) do
    opts[k] = v
  end
  return AddressEntity.new(self._client, opts)
end
...
function AddressEntity:mark_deleted()
  self._deleted = true
end


function AddressEntity:deleted()
  return true == self._deleted
end


function AddressEntity:data_set(args)
  if args ~= nil then
...
function AddressEntity:data_get()
  self._utility.feature_hook(self._entctx, "GetData")
  return vs.clone(self._data)
end


function AddressEntity:match_set(args)
  if args ~= nil then
...
function AddressEntity:match_get()
...
function AddressEntity:stream(action, args, callopts)
...
function AddressEntity:_run_op(ctx, post_done)
  local utility = self._utility
